...
export const outboundRouter = createTRPCRouter({
	getAbsoluteFilteredTopCandidates: publicProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const topCandidatesWithScores = topCandidates
				...
				.filter((candidate) => (input.showActiveGithub ? candidate.activeGithub : true))
				.filter((candidate) =>
	...
	getPendingSimilarProfiles: publicProcedure.query(async ({ ctx }) => {
		const pendingSimilarProfiles = await ctx.db.select().from(schema.profileQueue);
	...
	findFilteredCandidates: publicProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			try {
				console.log("Similar technologies arrays:", similarTechnologiesArrays.flat());
...
Respond only with a JSON object that has four fields: "standardizedTechs", "standardizedSpecialties", and "standardizedFeatures", "companyNames". Each should be an array of standardized terms.
        `,
			...
			const standardizedTechs: string[] =
				standardizedResponse.standardizedTechs?.map((tech: string) => tech.toLowerCase()) ??
				[];
			...
			console.log("Standardized technologies:", JSON.stringify(standardizedTechs, null, 2));
			...
			const allTechnologiesToSearch: {
				...
				standardizedTechs.map(async (tech) => await querySimilarTechnologies(tech))
			);

			console.log(allTechnologiesToSearch.map((s) => s.map((t) => t.technology)));

			const allFeaturesToSearch: {
				score: number;
				feature: string;
			}[][] = await Promise.all(
				standardizedFeatures.map(async (feature) => await querySimilarFeatures(feature))
			);
			...
			console.log(`allFeaturesToSearch: ${JSON.stringify(allFeaturesToSearch, null, 2)}`);
			console.log(
			...
			for (const company of companiesList) {
				...
				if (standardizedSpecialties.length > 0) {
					...
					for (const similarSpecialties of allSpecialtiesToSearch) {
						const hasMatchingSpecialty = similarSpecialties.some(({ specialty }) =>
						...
						if (hasMatchingSpecialty) {
							...
							similarSpecialties
								.filter(({ specialty }) => company.specialties?.includes(specialty))
								.map((res) => (score += res.score)) ?? 0;
						}
					}
			...
			return {
				...
				filters: [
					...standardizedTechs,
					...Array.from(new Set([...standardizedFeatures, ...standardizedSpecialties])),
				],
			};
		}),
...
`,
            ...
            "Or": false
				...
				let responseCompanyNames =
					response.companyNames.length > 0 ? response.companyNames : companyNames;
				...
				if (!companiesDB || companiesDB.length === 0) {
					console.error("No companies found or an error occurred during the DB query.");
					return {
						valid: true,
						message: "",
						companies: [],
		...
		.query(async ({ ctx, input }) => {
			const scoredCandidates = await ctx.db.query.candidates.findMany({
				where: (candidate, { and, inArray, eq }) =>
					and(inArray(candidate.id, input.ids), eq(candidate.cookdReviewed, true)),
			});
			return scoredCandidates;
		}),
	downloadAsCsv: publicProcedure.input(z.array(z.any())).mutation(async ({ input }) => {
		try {
			const csvData = input.map((candidate) => {
				...
				return {
					name: data.name || "",
					email: data.email || "",
					githubUrl: data.githubUrl || `https://github.com/${data.githubLogin}`,
					...
					score: candidate.score ? parseFloat(candidate.score).toFixed(4) : "",
