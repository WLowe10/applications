...
import { people, companiesVector, education } from "../../server/db/schemas/users/schema";
...
async function insertEducationEmbedding(personId: string, school: string, fieldOfStudy?: string) {
	try {
		const schoolVector = await getEmbedding(school);
		const fieldOfStudyVector = fieldOfStudy ? await getEmbedding(fieldOfStudy) : null;
...
async function processLinkedInAndBiosData() {
	...
	try {
		...
		console.log(`[processLinkedInAndBiosData] Found ${allPeople.length} people with data.`);
		...
		console.error("[processLinkedInAndBiosData] Error during processing:", error);
	}
}
