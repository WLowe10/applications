...
import { people, jobTitles, skills } from "../../server/db/schemas/users/schema";
...
async function updateLocationVector(
	...
	try {
		...
		await db.update(people).set({ locationVector }).where(eq(people.id, personId));
	...
	const unprocessedPeople = await db
		...
		.where(
			and(
				...
				not(exists(db.select().from(jobTitles).where(eq(jobTitles.personId, people.id)))),
				not(exists(db.select().from(skills).where(eq(skills.personId, people.id))))
			)
		);
	...
	for (const batch of batches) {
		await Promise.all(
			batch.map(async (person) => {
				console.log(`[updateMissingVectors] Processing person ID: ${person.id}`);
				...
				if (Array.isArray(person.topTechnologies) && person.topTechnologies.length > 0) {
