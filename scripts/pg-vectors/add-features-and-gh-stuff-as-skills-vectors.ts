...
async function upsertSkill(personId: string, skill: string) {
	try {
		...
		console.log(`[upsertSkill] Inserted/upserted skill "${skill}" for person ID: ${personId}`);
	} catch (error) {
		console.error(
...
async function upsertSkillsAndFeatures() {
	...
	console.log(`[upsertSkillsAndFeatures] Found ${targetPeople.length} people to process.`);

	if (targetPeople.length === 0) {
		console.log("[upsertSkillsAndFeatures] No people found with the specified criteria.");
		return;
	}
	...
	for (const [batchIndex, batch] of batches.entries()) {
		...
		console.log(`[upsertSkillsAndFeatures] Completed processing batch ${batchIndex + 1}.`);
	}

	console.log("[upsertSkillsAndFeatures] Upsert process completed successfully.");
}
...
upsertSkillsAndFeatures()
	...
	.catch((error) => {
		console.error("[upsertSkillsAndFeatures] Error during upsert process:", error);
		process.exit(1);
	});
