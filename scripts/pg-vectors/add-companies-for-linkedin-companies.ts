...
async function processLinkedInData() {
	...
	try {
		...
		const allPeople = await db.select().from(people).where(isNotNull(people.linkedinData));

		console.log(`[processLinkedInData] Found ${allPeople.length} people with LinkedIn data.`);
		...
		for (let i = 0; i < batches.length; i++) {
			console.log(`[processLinkedInData] Processing batch ${i + 1} of ${batches.length}`);
			...
			await Promise.all(
				batch.map(async (person) => {
					...
					const companies = ((linkedinData as any).positions?.positionHistory || [])
						.map((job: any) => job.companyName)
						.filter(Boolean);

					// Extract schools and fields of study from LinkedIn data
					const educationHistory = (linkedinData as any).schools?.educationHistory || [];
					...
					await Promise.all(
						companies.map((company: any) => upsertCompanyEmbedding(personId, company))
					);

					// Insert or update school embeddings
					await Promise.all(
						schoolsList.map((school: any) => upsertSchoolEmbedding(personId, school))
					);
