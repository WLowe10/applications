	...
	for (let i = 0; i < allUsers.length; i++) {
		...
		try {
			...
			if (companyEmbeddings.length > 0) {
				updates.averageCompanyVector = computeAverageEmbedding(companyEmbeddings);
			}
			...
			const fieldOfStudyEmbeddings = userFieldsOfStudy.map((field) => field.vector);
			if (fieldOfStudyEmbeddings.length > 0) {
				updates.averageFieldOfStudyVector = computeAverageEmbedding(fieldOfStudyEmbeddings);
			}

			// Job Titles
			const userJobTitles = allJobTitles.filter(
				(jobTitle) => jobTitle.personIds && jobTitle.personIds.includes(personId)
			);
			const jobTitleEmbeddings = userJobTitles.map((jobTitle) => jobTitle.vector);
			if (jobTitleEmbeddings.length > 0) {
				updates.averageJobTitleVector = computeAverageEmbedding(jobTitleEmbeddings);
			}

			// Location
			const userLocations = allLocations.filter(
				(location) => location.personIds && location.personIds.includes(personId)
			);
			const locationEmbeddings = userLocations.map((location) => location.vector);
