			...
			for (const personId of field.personIds) {
				try {
					...
					if (fieldEmbeddings.length > 0) {
						const averageFieldOfStudyVector = computeAverageEmbedding(fieldEmbeddings);

						// Update the person's record
						await updatePersonEmbeddings(personId, {
							averageFieldOfStudyVector,
						});
						console.log(`Updated field of study embeddings for user: ${personId}`);
					} else {
						console.log(`No field of study embeddings to update for user: ${personId}`);
					}
				} catch (error) {
					console.error(
