...
export const findUsersByJobTitle = async (inputJobTitle: string) => {
	...
	const similarities: { personId: string; similarity: number }[] = similarJobTitles.map((jt) => ({
	...
	console.log(`[3.5] Found ${topSimilarities.length} similar job titles after filtering`);
	...
	const result = users.map((user) => ({
		...
		similarity: topSimilarities.find((s) => s.personId === user.id)?.similarity || 0,
	}));
