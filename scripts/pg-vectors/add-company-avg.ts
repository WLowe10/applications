			...
			for (const personId of company.personIds) {
				...
				try {
					...
					if (companyEmbeddings.length > 0) {
						const averageCompanyVector = computeAverageEmbedding(companyEmbeddings);
					...
					console.error(`Error updating embeddings for user ${personId}:`, error);
				}
			}
		}
	}
	console.log(`Finished processing all users. Total processed: ${processedPersonIds.size}`);
}
