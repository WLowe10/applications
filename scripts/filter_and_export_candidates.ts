...
async function main() {
	...
	try {
		...
		const filteredCandidates = candidates.filter((candidate) => candidate.data?.githubLogin);
		...
		const csvData = filteredCandidates.map((candidate) => {
			...
			return {
				...
				contributionYears: data?.contributionYears ? data.contributionYears.join(", ") : "",
				...
				activeGithubScore: activeGithubScore ? activeGithubScore.toFixed(4) : "",
			};
		});
