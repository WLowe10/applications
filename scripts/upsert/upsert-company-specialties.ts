...
const main = async () => {
	...
	console.log(`Fetched ${companies.length} companies. Starting to process each company...`);

	for (const company of companies) {
		console.log(`Processing company: ${company.name} (ID: ${company.id})`);

		const specialtyPromises = (company.specialties ?? []).map(async (specialty) => {
			if (/^[\x00-\x7F]+$/.test(specialty)) {
				...
				console.log(`Upserting specialty: "${specialty}" for company ID: ${company.id}`);
		...
		console.log(`Finished processing company: ${company.name} (ID: ${company.id})`);
	}
