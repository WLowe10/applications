...
const processTechnologies = async (candidates: InferSelectModel<typeof candidatesTable>[]) => {
	...
	for (let j = 0; j < sortedTechnologies.length; j += batchSize) {
		...
		i += batchSize;
		console.log(`Processed ${i} technologies, ${(i / sortedTechnologies.length) * 100}% done`);
	}
};
