...
const calculateUserScore = (user: any, isInNewYork: boolean): { total: number; breakdown: any } => {
	const logScale = (value: number, base: number = 10) => Math.log(value + 1) / Math.log(base);
	...
	const experienceScore = Math.min(user.contributionYears?.length || 0, 10) * experienceWeight;
	const contributionsScore =
		logScale(user.totalCommits + user.restrictedContributions) * contributionsWeight;
	...
	// Twitter scores
	const twitterFollowerScore = logScale(user.twitterFollowerCount || 0) * twitterFollowerWeight;
	const twitterFollowRatio = (user.twitterFollowerCount || 0) / (user.twitterFollowingCount || 1);
	const twitterFollowRatioScore = logScale(twitterFollowRatio) * twitterFollowRatioWeight;
...
async function searchAndScoreCandidates() {
    ...
    `;
		...
		scoredUsers.slice(0, 100).forEach((user) => {
			...
			resultLines.push(`  similarityScore: ${user.score.similarityScore.toFixed(2)}`);
...
searchAndScoreCandidates().catch((error) => console.error("Error in main function:", error));
