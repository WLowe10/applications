...
async function updateBigTechStatus() {
	...
	console.log(`[updateBigTechStatus] Found ${peopleToProcess.length} people to process.`);
	...
	for (const [batchIndex, batch] of batches.entries()) {
		...
		await Promise.all(
			batch.map(async (person) => {
				const { id: personId, twitterBio, organizations, linkedinData } = person;
		...
		console.log(`[updateBigTechStatus] Completed processing batch ${batchIndex + 1}.`);
	}
