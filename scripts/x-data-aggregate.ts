...
export async function resolveTcoLink(url: string): Promise<string> {
	try {
		...
		if (!response.ok) {
			console.error(`Failed to resolve t.co link: ${url}. Status: ${response.status}`);
			return url;
		}
...
export async function getPopularTweets(username: string, limit: number = 100) {
	...
	try {
		...
		allTweets.sort((a, b) => {
			const engagementA = a.retweet_count + a.favorite_count + (a.reply_count || 0);
			const engagementB = b.retweet_count + b.favorite_count + (b.reply_count || 0);
		...
		const popularTweets = await Promise.all(
			allTweets.map(async (tweet: any) => {
				...
				const tcoLinks: string[] = tweet.full_text.match(/https?:\/\/t\.co\/\w+/g) || [];
				const resolvedLinks: string[] = await Promise.all(tcoLinks.map(resolveTcoLink));
				...
				return {
					...
					engagement:
						tweet.retweet_count + tweet.favorite_count + (tweet.reply_count || 0),
				};
			})
		);
