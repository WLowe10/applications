...
export async function fetchUserFromGitHub(username: string): Promise<GitHubUser | null> {
	const query = `
    query($login: String!) {
      user(login: $login) {
        login
  ...
  `;
	...
	try {
		...
		const externalContributions = user.contributionsCollection.commitContributionsByRepository
		...
		return {
			...
			languages: Object.fromEntries(
				Object.entries(languagesMap).sort((a, b) => b[1].repoCount - a[1].repoCount)
			),
			uniqueTopics: topics,
...
export async function getOrFetchUser(username: string): Promise<GitHubUser | null> {
	let user = null;
...
export function calculateSimilarityScore(
	...
	let score = 0;
	score += user.organizations?.some((org) => similarities.organizations.includes(org.login))
		? 1
		: 0;
	score += similarities.commonLocations.includes(user.normalizedLocation || "") ? 2 : 0;
	const userLanguages = Object.keys(user.languages || {});
	score += similarities.commonLanguages.filter((lang) => userLanguages.includes(lang)).length;
	score +=
		1 /
		(1 +
...
async function main(inputUserLogins: string[]) {
	try {
		...
		console.log("Similarities among input users:", JSON.stringify(similarities, null, 2));
