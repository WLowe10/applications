...
async function fetchGitHubUserData(username: string): Promise<any | null> {
  ...
  `;
	...
	try {
		...
		const githubLanguages: Record<string, { repoCount: number; stars: number }> = {};
		...
		const normalizedLocation = await getNormalizedLocation(userData.location || "");
		const normalizedCountry = await getNormalizedCountry(userData.location || "");
		...
		return {
			...
			miniSummary: linkedinData ? await generateMiniSummary(linkedinData) : null,
			...
			jobTitles: linkedinData
				? (linkedinData as any).positions?.positionHistory.map((p: any) => p.title) || []
				: [],
			topFeatures: linkedinData ? (await gatherTopSkills(linkedinData)).features : [],
			isEngineer: linkedinData ? (await gatherTopSkills(linkedinData)).isEngineer : null,
			createdAt: new Date(),
			...
			totalCommits,
			restrictedContributions: userData.contributionsCollection.restrictedContributionsCount,
			...
			sponsoredProjects: userData.sponsors.nodes.map((sponsor: any) => sponsor.login),
...
async function checkWhopStatus(email: string): Promise<WhopResponse> {
	try {
		const response = await fetch(
			`https://api.whop.com/api/v3/sales/check_email?email=${encodeURIComponent(email)}`,
			{
				method: "GET",
				headers: {
					Authorization: "Bearer c1f659631e81c7e2e40063e0f483f15e2b5254b267c7794a",
					Cookie: "__cf_bm=rnVqtFbXpJzNgbYYoJZN8ydG4EMrioF7rz3pFqEQ98s-1726681291-1.0.1.1-ZKFkZRiGFVNq3zO1y5O2S_o96dCE84CS9nx30GtSH9qoq0bbm2tSRZAeZ9jVc1m.xFR.ApRTVWSHG1PFzfL3yg",
				},
			}
		);
...
async function updatePersonEmbeddings(personId: string, updates: any) {
	await db.update(schema.people).set(updates).where(eq(schema.people.id, personId));
}

const companyMutexes = new Map<string, Mutex>();

async function upsertData(table: any, columnName: string, value: string, personId: string) {
...
async function processUser(userData: any) {
	...
	for (const followingUser of followingList) {
		if (!processedUsers.has(followingUser.githubLogin)) {
			await fetchGitHubUserData(followingUser.githubLogin).then((followingUserData) => {
...
async function computeAndStoreVectorsForUser(userData: any) {
	...
	if (allSkills.length > 0) {
		...
		const validSkillVectors = skillVectors.filter((v): v is number[] => v !== null);
	...
	if (userData.jobTitles && userData.jobTitles.length > 0) {
		...
		const validJobTitleVectors = jobTitleVectors.filter((v): v is number[] => v !== null);
		if (validJobTitleVectors.length > 0) {
			updates.averageJobTitleVector = computeAverageEmbedding(validJobTitleVectors);
		}
	}
	...
	const companies = [
		...
		...(userData.linkedinData?.positions?.positionHistory.map((p: any) => p.companyName) || []),
	].filter(Boolean);
	if (companies.length > 0) {
		...
		const validCompanyVectors = companyVectors.filter((v): v is number[] => v !== null);
		if (validCompanyVectors.length > 0) {
			updates.averageCompanyVector = computeAverageEmbedding(validCompanyVectors);
		}
	}
	...
	if (userData.linkedinData && userData.linkedinData.education) {
		const schoolVectors = await Promise.all(
			userData.linkedinData.education.educationHistory.map(
				(edu: any) =>
					edu.school && upsertData(schema.schools, "school", edu.school, userData.id)
			)
		);
		const fieldOfStudyVectors = await Promise.all(
			userData.linkedinData.education.educationHistory.map(
				(edu: any) =>
					edu.fieldOfStudy &&
					upsertData(schema.fieldsOfStudy, "fieldOfStudy", edu.fieldOfStudy, userData.id)
			)
		);
		const validSchoolVectors = schoolVectors.filter((v): v is number[] => v !== null);
		...
		if (validFieldOfStudyVectors.length > 0) {
			updates.averageFieldOfStudyVector = computeAverageEmbedding(validFieldOfStudyVectors);
		}
	}
...
async function updateExistingUser(userId: string, updates: any) {
	await db.update(schema.people).set(updates).where(eq(schema.people.id, userId));
