...
export const googleSearch = async (query: string) => {
	...
	for (let start = 1; start < maxResults; start += resultsPerPage) {
		const url = `https://www.googleapis.com/customsearch/v1?q=${encodeURIComponent(
			query
		)}&key=${encodeURIComponent(apiKey)}&cx=${encodeURIComponent(cseId)}&start=${start}`;
	...
	console.log(`Google search completed. Found ${allLinkedinUrls.length} LinkedIn URLs.`);
...
export const processUrls = async (urls: string[]) => {
	...
	const promises = urls.map(async (url) => {
		...
		if (profileData && profileData.success) {
			...
			const name = profileData.person.firstName + " " + profileData.person.lastName;
...
const main = async () => {
	...
	for (const query of queries.slice(0, 200)) {
		...
		console.log(`Number of URLs returned that contain www.linkedin.com/in: ${urls.length}`);
