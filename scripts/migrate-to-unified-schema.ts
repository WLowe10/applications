...
async function fetchUserProfile(username: string): Promise<any> {
	...
	console.log(`[fetchUserProfile] Successfully fetched data for username: ${username}`);
...
async function updateLocationVector(
	...
	} else if (location && location.trim() !== "" && location.trim().toUpperCase() !== "UNKNOWN") {
		chosenLocation = location.trim();
	}

	if (!chosenLocation || chosenLocation.toUpperCase() === "UNKNOWN") {
		console.log(`[updateLocationVector] No valid location found for person ID: ${personId}`);
		return;
	}

	try {
		const locationVector = await getEmbedding(chosenLocation);
		await db.update(people).set({ locationVector }).where(eq(people.id, personId)).execute();
		console.log(`[updateLocationVector] Updated location vector for person ID: ${personId}`);
	} catch (error) {
		console.error(
...
async function insertJobTitle(personId: string, title: string) {
	try {
		...
		await db
			...
			.execute();
		console.log(`[insertJobTitle] Inserted job title "${title}" for person ID: ${personId}`);
	} catch (error) {
		console.error(
...
async function insertSkill(personId: string, skill: string) {
	try {
		...
		await db
			...
			.execute();
		console.log(`[insertSkill] Inserted skill "${skill}" for person ID: ${personId}`);
	} catch (error) {
		console.error(
...
async function migrateData() {
	...
	console.log(`[migrateData] Retrieved ${githubUserRows.length} unprocessed GitHub users`);
		...
		await Promise.all(
			batch.map(async (githubUser) => {
				...
				try {
					...
					if (Array.isArray(personData.jobTitles) && personData.jobTitles.length > 0) {
				...
				console.log(`[migrateData] Marked GitHub user as processed: ${githubUser.login}`);
			})
	...
	console.log(`[migrateData] Retrieved ${unprocessedCandidates.length} unprocessed candidates`);
		...
		await Promise.all(
			batch.map(async (candidate) => {
				...
				await db
					...
					.set({ processed: true })
					.where(eq(candidates.id, candidate.id));
				console.log(`[migrateData] Marked candidate as processed: ${candidate.url}`);
			})
		);
	}
	...
	await processTwitterAccounts(schema.whopTwitterAccounts, "whopTwitterAccounts");
	await processTwitterAccounts(schema.whopTwitterFollowers, "whopTwitterFollowers");
	await processTwitterAccounts(schema.whopTwitterFollowing, "whopTwitterFollowing");
