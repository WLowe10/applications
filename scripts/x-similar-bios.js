...
async function queryAllEmbeddingsFromNamespace(namespace) {
	try {
		...
		console.log(`Retrieved ${matches.length} embeddings from namespace ${namespace}.`);
		...
		console.error(`Error querying embeddings from namespace ${namespace}:`, error);
		throw error;
	}
}
...
async function queryWithAveragedEmbedding(avgEmbedding) {
	try {
		...
		for (const result of results) {
			console.log(
				`https://x.com/${result.metadata.username}, Score: ${result.score.toFixed(4)}`
			);
		}
...
async function main() {
	try {
		...
		const embeddings = await queryAllEmbeddingsFromNamespace("x-bio-staff-swe-infra");
