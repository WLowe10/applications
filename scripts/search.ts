...
const roleDescriptions = {
	...
	mobile: "mobile app development. This includes roles like iOS Engineer, Android Engineer, Mobile App Developer, or Cross-platform Mobile Developer (e.g., React Native or Flutter).",
	android:
...
async function fetchCandidatesWithCursor(
	...
	console.log(`Fetching candidates for ${company.name} with cursor: ${JSON.stringify(cursor)}`);
	try {
...
async function fetchAllCandidates(company: any) {
	...
	console.log(`Total candidates fetched for ${company.name}: ${allCandidates.length}`);
...
async function checkCompanyExperience(positionHistory: any[], company: any) {
	...
	const relevantPositions = positionHistory
		.filter((position: any) => {
			...
			return (
				companyMatch &&
				((startYear && startYear >= company.startYear && startYear <= company.endYear) ||
					(endYear && endYear >= company.startYear && endYear <= company.endYear))
			);
		})
		.map((position: any) => ({
	...
	console.log(`Relevant positions found for ${company.name}: ${relevantPositions.length}`);
...
Here are the positions to evaluate: ${condition}`,
		...
		console.error(`Error processing OpenAI response for ${company.name}:`, error);
		return false;
	}
}
...
async function main() {
	...
	async function processCompany(company: any) {
		...
		if (fs.existsSync(fileName) && fs.statSync(fileName).size > 0) {
			console.log(`Skipping ${company.name} (${company.type}) as it is already processed.`);
			return;
		}
		...
		const checkPromises = allCandidates.map(async (candidate) => {
			...
			if (linkedinData && linkedinData.positions && linkedinData.positions.positionHistory) {
	...
	const metaFileName = `meta/engineering_product_${
		companies.find((c) => c.name === "Meta" && c.type === "engineering_product")?.startYear
	}-${
		companies.find((c) => c.name === "Meta" && c.type === "engineering_product")?.endYear
	}.json`;

	const facebookFileName = `facebook/engineering_product_${
		companies.find((c) => c.name === "Facebook" && c.type === "engineering_product")?.startYear
	}-${
		companies.find((c) => c.name === "Facebook" && c.type === "engineering_product")?.endYear
	}.json`;

	if (results[metaFileName] && results[facebookFileName]) {
		results[metaFileName] = results[metaFileName].concat(results[facebookFileName]);
		delete results[facebookFileName];

		// Write combined Meta results to file
		fs.writeFileSync(metaFileName, JSON.stringify(results[metaFileName], null, 2));
	}
