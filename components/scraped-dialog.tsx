...
export function ScrapedDialog() {
	...
	const initializeFilterWeights = () => {
		...
		const totalWeight = Object.values(weights)
			.map((w) => (typeof w === "number" ? w : Object.values(w).reduce((a, b) => a + b, 0)))
	...
	const handleWeightChange = (filterType: string, value: number, skillName?: string) => {
		let oldWeight;
		if (skillName) {
			oldWeight = (filterWeights.skills as { [key: string]: number })[skillName];
		} else {
		...
		Object.keys(newWeights).forEach((key) => {
			if (key !== filterType && key !== "skills" && typeof newWeights[key] === "number") {
	...
	const getPendingSimilarProfilesQuery = api.outbound.getPendingSimilarProfiles.useQuery(
		undefined,
		{
	...
	useEffect(() => {
		...
		if (getPendingSimilarProfilesQuery.data && getPendingSimilarProfilesQuery.data[0]) {
			setLoading(true);
		}
		if (getPendingSimilarProfilesQuery.data && getPendingSimilarProfilesQuery.data[0]?.error) {
	...
	const handleManualUrlsChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
		...
		const { linkedinUrls, githubUrls, twitterUsernames } = extractUrls(e.target.value);
		...
		setProfileUrls((prev) =>
			[...prev.filter((url) => url.mode === "UPLOAD"), ...newProfileUrls].filter(
				(url, index, self) =>
					index === self.findIndex((t) => t.url === url.url && t.type === url.type)
			)
		);
	...
	const handleFileProcessing = async (file: File) => {
		...
		reader.onload = (event) => {
			...
			const { linkedinUrls, githubUrls, twitterUsernames } = extractUrls(content);
			if (linkedinUrls.length > 0 || githubUrls.length > 0 || twitterUsernames.length > 0) {
				const newProfileUrls: ProfileUrl[] = [
					...linkedinUrls.map((url) => ({
						type: "linkedin" as const,
				...
				setError("No valid LinkedIn, GitHub, or Twitter URLs found in the file.");
			}
		};
		reader.onerror = () => {
	...
	const findSimilarProfiles = async (profileUrls: ProfileUrl[]) => {
		...
		const linkedinUrls = profileUrls
			...
			.map((url) => url.url);
		const githubUrls = profileUrls.filter((url) => url.type === "github").map((url) => url.url);
	...
	}, [showGithub, showLinkedin, showTwitter, showWhop, showActiveGithub, showMatchingLocation]);

	return (
		<ScrollArea>
			<Card size="5" className="my-36 p-4 w-[90%] sm:w-[75%] lg:w-[800px] mx-auto shadow-md">
				<Toaster richColors className="z-[99999]" />
				<Heading size={"6"}>Candidate Search</Heading>
				<Text className="text-primary/60">Provide details to search for candidates.</Text>
				<Separator />
				...
				<Flex direction="column" gap="4" mt="4">
					...
					<Flex direction="column" gap="2">
						...
						<Flex wrap="wrap" gap="2" mt="2">
							...
							{filters &&
								...
								filters.otherCompanyNames.length > 0 &&
								filters.otherCompanyNames.map((company: string, index: number) => (
									<Button
										key={index}
										variant="surface"
										size="2"
					...
					<Flex justify="end" gap="2">
						...
						<Button
							...
							onClick={() => {
								if (loading) return;
								if (query.trim() === "" && profileUrls.length === 0 && !filters) {
					...
					<DialogRoot>
						...
						<DialogContent>
							...
							<div className="flex flex-wrap gap-2">
								...
								>
									{showWhop ? <Check className="size-4 text-green-500" /> : null}
									<Image
										src={"/whop.png"}
										width={30}
										height={30}
