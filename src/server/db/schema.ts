...
export const candidates = pgTable("candidates", {
	...
	companyId: varchar("company_id", { length: 255 }).references(() => company.id), // not unique until we make the matches (weight similarity and stuff json object)
	...
	isJobTitleAvgInVectorDB: boolean("is_job_title_avg_in_vector_db").default(false),
...
export const githubUsers = pgTable("github_users", {
	...
	languages: jsonb("languages").$type<Record<string, { repoCount: number; stars: number }>>(),
...
export const people = pgTable(
	"people",
	{
		...
		githubLanguages:
			jsonb("github_languages").$type<Record<string, { repoCount: number; stars: number }>>(),
		uniqueTopics: jsonb("unique_topics").$type<string[]>(),
...
export const education = pgTable(
	...
	(table) => ({
		uniqueSchoolPerPerson: unique().on(table.personId, table.school, table.fieldOfStudy),
