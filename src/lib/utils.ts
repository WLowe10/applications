...
export function jsonArrayContainsAny<T extends AnyColumn>(column: T, values: T["_"]["data"]): SQL {
	return sql`
    EXISTS (
      SELECT 1
...
export function jsonArrayContains<T extends AnyColumn>(column: T, values: string[]): SQL {
	return sql`
    (
