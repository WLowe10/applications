...
async function querySimilarPeopleByEmbedding(
	...
	try {
		...
		const similarity = sql<number>`1 - (${cosineDistance(vectorColumn, embedding)})`;
...
const generateEmbeddingsWithLogging = async (data: string[], type: string) => {
	...
	const embeddings = await Promise.all(
		data.map(async (item, index) => {
			try {
				console.log(`Generating ${type} embedding for item ${index + 1}/${data.length}`);
				const embedding = await getEmbedding(item);
				console.log(`Successfully generated ${type} embedding for item ${index + 1}`);
				return embedding;
			} catch (error) {
				console.error(`Error generating ${type} embedding for item ${index + 1}:`, error);
				throw error;
			}
		})
	);
...
function calculateExperienceBounds(
	...
	candidates.forEach((candidate) => {
		...
		if (candidate.linkedinData?.positions?.positionHistory) {
			candidate.linkedinData.positions.positionHistory.forEach((position: any) => {
...
async function computeAverageEmbedding(embeddings: number[][]): Promise<number[] | null> {
...
async function processGitHubUrls(githubUrls: string[], insertId: string) {
	...
	const addToAllSimilarPeople = (personId: string, score: number, attribution: string) => {
		if (validMetrics.includes(attribution)) {
			const existingPerson = allSimilarPeople.find((p) => p.personId === personId);
			if (existingPerson) {
	...
	validMetrics.forEach((metric, index) => {
		...
		console.log(`Processing ${attribution} data (${index + 1}/${validMetrics.length})...`);
	...
	const topSimilarPeople = allSimilarPeople.sort((a, b) => b.score - a.score).slice(0, 2000);
	...
	const scoredCandidates = topCandidates
		.map((person) => {
			const similarityData = topSimilarPeople.find((p) => p.personId === person.id);
			return {
				data: { ...person },
	...
	console.log(`Processed and normalized ${normalizedCandidates.length} top candidates.`);
...
async function insertPersonFromGithub(profileData: any) {
	...
	const totalExternalCommits = contributionsCollection.restrictedContributionsCount || 0;
	...
	for (const skill of skills) {
		try {
			const vector = await upsertData(schema.skillsNew, "skill", skill, "vector", personId);
			skillVectors.push(vector);
		} catch (error) {
			console.error(`Error processing skill "${skill}" for person ID: ${personId}`, error);
		}
	}
	...
	try {
		...
		console.log(`Person ${name || login} inserted into the database. Person ID: ${personId}`);
	} catch (e) {
		console.error(`Failed to insert person ${name || login} into the database.`, e);
	}
...
async function insertPersonFromLinkedin(profileData: any) {
	...
	const schoolNames = educationHistory.map((education: any) => education.schoolName) as string[];
	...
	for (const school of schoolNames) {
		try {
			const vector = await upsertData(schema.schools, "school", school, "vector", personId);
			schoolVectors.push(vector);
		} catch (error) {
			console.error(`Error processing school "${school}" for person ID: ${personId}`, error);
		}
	}
	...
	const averageFieldOfStudyVector = await computeAverageEmbedding(fieldOfStudyVectors);
...
async function processLinkedinUrls(profileUrls: string[], insertId: string) {
	...
	for (let i = 0; i < profileUrls.length; i += batchSize) {
		...
		const batchResults = await Promise.all(
			batch.map(async (profileUrl) => {
				...
				if (!person) {
					console.log(`Person not found for URL: ${profileUrl}. Scraping and inserting.`);
					...
					if (scrapedData && scrapedData.success) {
						...
						console.error(`Failed to scrape or insert person for URL: ${profileUrl}`);
					}
				}
		...
		inputPeople.push(
			...batchResults.filter((p): p is InferSelectModel<typeof people> => p !== undefined)
		);
	}
	...
	const inputPersonAverageEmbeddings = await Promise.all(
		inputPeople.map(async (person) => {
			...
			const fieldOfStudyEmbedding = person.averageFieldOfStudyVector as number[] | null;
...
export const querySimilarTechnologies = async (inputSkill: string, topK: number = 250) => {
	try {
		console.log(`[1] Starting search for similar technologies to: "${inputSkill}"`);
...
export const querySimilarLocations = async (inputLocation: string, topK: number = 500) => {
	try {
		console.log(`[1] Starting search for similar locations to: "${inputLocation}"`);
...
export const querySimilarCompanies = async (inputCompany: string, topK: number = 500) => {
	try {
		console.log(`[1] Starting search for similar locations to: "${inputCompany}"`);
...
export const querySimilarJobTitles = async (inputJobTitle: string, topK: number = 500) => {
	try {
		console.log(`[1] Starting search for similar job titles to: "${inputJobTitle}"`);
...
export const querySimilarSchools = async (inputSchool: string, topK: number = 500) => {
	try {
		...
		const similarity = sql<number>`1 - (${cosineDistance(schema.schools.vector, embedding)})`;
...
export const querySimilarFieldsOfStudy = async (inputFieldOfStudy: string, topK: number = 500) => {
	try {
		console.log(`[1] Starting search for similar fields of study to: "${inputFieldOfStudy}"`);
...
export async function processFilterCriteria(filterCriteria: FilterCriteria) {
	...
	let companyIds =
		filterCriteria.companyIds.values.length > 0 ? filterCriteria.companyIds.values : ["NONE"];
	...
	const [similarTechnologiesResults, similarCompanies] = await Promise.all([
		filterCriteria.skills.values.length > 0
			...
			: Promise.resolve([]),
		Promise.all(companyNames.map((companyName) => querySimilarCompanies(companyName))),
	]);
	...
	similarTechnologiesResults.forEach(({ skill, weight, similarTechnologies }) => {
	...
	if (filterCriteria.location.value) {
		similarLocations = await querySimilarLocations(filterCriteria.location.value);
	}
	...
	if (filterCriteria.schools.values.length > 0) {
		similarSchools = (
			await Promise.all(
				filterCriteria.schools.values.map((school) => querySimilarSchools(school))
			)
		).flat();
	}
	...
	if (filterCriteria.fieldsOfStudy.values.length > 0) {
		similarFieldsOfStudy = (
			await Promise.all(
				filterCriteria.fieldsOfStudy.values.map((field) => querySimilarFieldsOfStudy(field))
			)
		).flat();
	}
	...
	const peopleDataMap = new Map(peopleData.map((person) => [person.id, person]));
	...
	similarTechnologiesResults.forEach(({ skill, weight, similarTechnologies }) => {
		const scores: number[] = similarTechnologies.map((tech) => tech.score || 0);
		const mean = calculateMean(scores);
		const stdDev = calculateStdDev(scores, mean);
	...
	combinedPersonIds.forEach((personId) => {
		...
		Object.keys(personSkills).forEach((skill) => {
			...
			const skillWeight =
				filterCriteria.skills.values.find((s) => s.skill === skill)?.weight || 0;
		...
		githubMetricsRawValues.totalCommits.push(githubMetricsPerPerson.totalCommits);
		githubMetricsRawValues.totalStars.push(githubMetricsPerPerson.totalStars);
		githubMetricsRawValues.totalRepositories.push(githubMetricsPerPerson.totalRepositories);
	...
	mostSimilarPeople.forEach((person) => {
		...
		Object.keys(criteriaWeights).forEach((criterion) => {
			if (criterion === "activeGithub") {
				...
				const { mean, stdDev } = calculateMeanAndStdDev(criterionRawScores.activeGithub);
	...
	console.log(`Scores: ${topCandidatesWithData.map((c) => c.score.toFixed(4)).join(", ")}`);
	...
	for (const resultsArray of resultsArrays) {
		for (const item of resultsArray) {
			...
			if (!mergedResultsMap[id]) {
				...
				mergedResultsMap[id].from = Array.isArray(item.from) ? item.from : [item.from];
				...
				const newFromSources = Array.isArray(item.from) ? item.from : [item.from];
...
function mergeArrayOfObjects(arr1: any[] = [], arr2: any[] = [], uniqueKeys: string[] = []): any[] {
			...
			.set({
				...
				allIdsResponse: mergedResults.map((res) => {
					...
					return {
						...
						githubUrl: data.githubLogin ? `https://github.com/${data.githubLogin}` : "",
