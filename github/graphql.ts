...
const fetchUserDetails = async (username: string, isTopLevelMember: boolean) => {
	const query = `
      query($login: String!) {
        user(login: $login) {
          login
...
async function processLinkedInData(personId: string, linkedinUrl: string) {
	...
	if (linkedinData) {
		...
		if (linkedinData.location) {
			const normalizedLocation = await getNormalizedLocation(linkedinData.location);
...
const processFollowingData = async (user: any) => {
	...
	return {
		...
		totalStars,
		totalForks,
		languages: Object.entries(languagesMap).sort((a, b) => b[1].repoCount - a[1].repoCount),
	...
	try {
		await db
			...
			.values({
				...
				followerToFollowingRatio: parseFloat(processedData.followerToFollowingRatio),
		...
		console.error(`Error inserting/updating data for user: ${processedData.login}`, error);
	}
};
...
async function upsertFieldOfStudyEmbedding(personId: string, fieldOfStudy: string) {
...
async function upsertLocationEmbedding(personId: string, normalizedLocation: string) {
