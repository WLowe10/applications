	...
	try {
		...
		if (usernames && usernames.length > 0) {
			...
			users = await db.query.githubUsers.findMany({
				where: (githubUsers, { inArray }) => inArray(githubUsers.login, usernames),
			});
			console.log(`Retrieved ${users.length} specified users from the database.`);
		} else {
		...
		console.log("Processing complete. Users written to 'scored_users.txt' sorted by score.");
	} catch (error) {
